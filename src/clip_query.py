...
def get_emb(dataloader, model):
    ...
    for i, (batch_mech_name, batch_text_input) in tqdm(enumerate(dataloader, 0)):
    ...
    return all_mech_name_list, all_emb
...
class PartQueryDataset(Dataset):
    ...
    def __init__(self, building_blocks, txt_processors):
        self.building_blocks_list = []
        for mech_name, info in building_blocks.items():
            self.building_blocks_list.append(mech_name)
    ...
    def __getitem__(self, idx):
        mech_name = self.building_blocks_list[idx]
        text_input = txt_processors["eval"](mech_name)

        return mech_name, text_input
...
if __name__ == '__main__':
    ...
    mech_names, embs = get_emb(queryloader, model)

    mech_part_dict = dict()
    mech_part_dict["mech_name"] = mech_names
    # mech_part_dict["part_name"] = part_names

    save_tensor(embs, '../processed_data/clip_mech_query_embs.pt')
    save_json(mech_part_dict, '../processed_data/clip_mech_query_metadata.json')
    print(mech_names)
    # print(part_names)
    print(embs.shape)
